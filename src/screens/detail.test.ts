...
describe('detail (cover)', () => {
  it('maps up/ok/down to open/stop/close', () => {
    ...
    const d = createDetail({ client: client as any, cache: new StateCache(), tile: tile('cover.garage') });
    mountInto(d);
    d.handleKey('up');
...
describe('detail (climate)', () => {
  it('up raises target temperature from the entity attribute', () => {
    ...
    const client = makeClient();
    const d = createDetail({ client: client as any, cache, tile: tile('climate.t') });
    const c = mountInto(d);
    d.handleKey('up');
...
describe('detail (light)', () => {
  it('right raises brightness and ok toggles', () => {
    ...
    const client = makeClient();
    const d = createDetail({ client: client as any, cache, tile: tile('light.a') });
    mountInto(d);
    d.handleKey('right');
